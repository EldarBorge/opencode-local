  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    gitlab: async (input) => {
      ...
      return {
        ...
        async getModel(
          sdk: ReturnType<typeof createGitLab>,
          modelID: string,
          options?: Record<string, any>,
        ) {
    ...
    if (provider) {
      ...
      for (const item of priority) {
        if (providerID === ProviderID.amazonBedrock) {
          ...
          if (globalMatch)
            return getModel(providerID, ModelID.make(globalMatch));
          ...
          if (region) {
            ...
            if (regionPrefix === "us" || regionPrefix === "eu") {
              ...
              if (regionalMatch)
                return getModel(providerID, ModelID.make(regionalMatch));
            }
          }
          ...
          for (const model of Object.keys(provider.models)) {
            if (model.includes(item))
              return getModel(providerID, ModelID.make(model));
          }
        }
      }
    }
